...
import {
    Radio, Building2, MapPin, Mail, Phone, Search,
    ChevronLeft, ChevronRight, Globe, X, Briefcase,
    Wifi, Signal, Satellite, Cable, Server,
    RefreshCw, Activity, Clock, Download, Filter
} from 'lucide-react';
...
const SUB_SECTOR_LABELS: Record<string, { label: string; icon: any; color: string }> = {
    fcc_cellular: { label: 'FCC Cellular', icon: Signal, color: 'blue' },
    fcc_microwave: { label: 'FCC Microwave', icon: Satellite, color: 'purple' },
    tower_infrastructure: { label: 'Tower Infra', icon: Radio, color: 'orange' },
    fiber_osp: { label: 'Fiber/OSP', icon: Cable, color: 'emerald' },
    wireless_rf: { label: 'Wireless RF', icon: Wifi, color: 'cyan' },
    network_engineering: { label: 'Network Eng', icon: Server, color: 'indigo' },
    it_infrastructure: { label: 'IT Infra', icon: Server, color: 'rose' },
    telecom_sales: { label: 'Sales', icon: Briefcase, color: 'amber' },
};

const SOURCE_CONFIG: Record<string, { label: string; color: string }> = {
    FCC_ULS: { label: 'FCC ULS', color: 'blue' },
    'Industry Database': { label: 'Industry DB', color: 'emerald' },
    indeed: { label: 'Indeed', color: 'indigo' },
    Indeed: { label: 'Indeed', color: 'indigo' },
    TelecomCareers: { label: 'TelecomCareers', color: 'purple' },
    LinkedIn: { label: 'LinkedIn', color: 'cyan' },
...
export default function TelecomPage() {
    ...
    const [subSectorCounts, setSubSectorCounts] = useState<Record<string, number>>({});
    ...
    const fetchStats = async () => {
        const [
            ...
            supabase.from('telecom_profiles').select('*', { count: 'exact', head: true }),
            supabase.from('telecom_profiles').select('*', { count: 'exact', head: true }).not('email', 'is', null).neq('email', ''),
            supabase.from('telecom_profiles').select('*', { count: 'exact', head: true }).not('phone', 'is', null).neq('phone', ''),
        ]);

        setStats({ total: total || 0, withEmail: withEmail || 0, withPhone: withPhone || 0 });

        // Count by sub_sector and source
        const { data: allRows } = await supabase
            .from('telecom_profiles')
            .select('sub_sector, sources');
        if (allRows) {
            const subCounts: Record<string, number> = {};
            const srcCounts: Record<string, number> = {};
            allRows.forEach((row: any) => {
                const sub = row.sub_sector || 'unknown';
                subCounts[sub] = (subCounts[sub] || 0) + 1;
                (row.sources || []).forEach((s: string) => {
                    srcCounts[s] = (srcCounts[s] || 0) + 1;
                });
            });
            setSubSectorCounts(subCounts);
    ...
    const fetchProfiles = useCallback(async () => {
        setLoading(true);
        let query = supabase.from('telecom_profiles').select('*', { count: 'exact' });
    ...
    const downloadCSV = () => {
        ...
        const headers = ['Name', 'Company', 'Title', 'Location', 'Sub-Sector', 'Email', 'Phone', 'Sources', 'Added'];
        const csvContent = [
            headers.join(','),
            ...profiles.map(p => [
        ...
        link.setAttribute('download', `telecom_profiles_${subSectorFilter}_${sourceFilter}.csv`);
    ...
    const getSubSectorStyle = (sub: string) => {
        const styles: Record<string, { bg: string; border: string; text: string }> = {
            fcc_cellular: { bg: 'bg-blue-500/10', border: 'border-blue-500/20', text: 'text-blue-400' },
            fcc_microwave: { bg: 'bg-purple-500/10', border: 'border-purple-500/20', text: 'text-purple-400' },
            tower_infrastructure: { bg: 'bg-orange-500/10', border: 'border-orange-500/20', text: 'text-orange-400' },
            fiber_osp: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
            wireless_rf: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/20', text: 'text-cyan-400' },
            network_engineering: { bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', text: 'text-indigo-400' },
            it_infrastructure: { bg: 'bg-rose-500/10', border: 'border-rose-500/20', text: 'text-rose-400' },
            telecom_sales: { bg: 'bg-amber-500/10', border: 'border-amber-500/20', text: 'text-amber-400' },
        };
    ...
    const getSourceStyle = (source: string) => {
        const styles: Record<string, { bg: string; border: string; text: string }> = {
            FCC_ULS: { bg: 'bg-blue-500/10', border: 'border-blue-500/20', text: 'text-blue-400' },
            'Industry Database': { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
            indeed: { bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', text: 'text-indigo-400' },
            Indeed: { bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', text: 'text-indigo-400' },
            TelecomCareers: { bg: 'bg-purple-500/10', border: 'border-purple-500/20', text: 'text-purple-400' },
            LinkedIn: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/20', text: 'text-cyan-400' },
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                        ...
                        Telecom & IT Intelligence
                    </h1>
                    <p className="text-neutral-400 font-medium mt-2">
                        {stats.total.toLocaleString()} profiles across cellular, microwave, fiber & tower infrastructure
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={downloadCSV}
                        className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-lg bg-blue-500 text-white hover:bg-blue-400 transition-colors shadow-lg shadow-blue-500/20"
                    >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'TOTAL PROFILES', value: stats.total, icon: Building2, color: 'blue' },
                    ...
                    { label: 'FCC LICENSES', value: (subSectorCounts['fcc_cellular'] || 0) + (subSectorCounts['fcc_microwave'] || 0), icon: Signal, color: 'cyan' },
                ].map(s => (
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-2 overflow-x-auto pb-1">
                    <span className="text-[10px] font-black text-neutral-500 uppercase tracking-wider shrink-0 mr-1">Sector:</span>
                    <button
                    ...
                    {Object.entries(SUB_SECTOR_LABELS).map(([key, info]) => {
                        const count = subSectorCounts[key] || 0;
                        ...
                        return (
                            ...
                            >
                                ...
                                {info.label} <span className="text-neutral-500 ml-0.5">({count.toLocaleString()})</span>
                            </button>
                        );
                    })}
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder="Search by company, title, name, or location..."
                        ...
                        className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-medium placeholder:text-neutral-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 transition" />
                </div>
                {(subSectorFilter !== 'all' || sourceFilter !== 'all' || search) && (
                    <button
                        onClick={() => { setSubSectorFilter('all'); setSourceFilter('all'); setSearch(''); setPage(0); }}
                        className="text-xs font-bold text-blue-400 hover:text-blue-300 px-4 py-2 bg-blue-500/10 rounded-xl border border-blue-500/20 transition-colors whitespace-nowrap"
                    ...
                    <tbody className="divide-y divide-white/[0.03]">
                        {loading ? Array(8).fill(0).map((_, i) => (
                            ...
                            <tr><td colSpan={5} className="px-6 py-20 text-center">
                                <Radio className="w-12 h-12 text-neutral-700 mx-auto mb-4" />
                                <p className="font-bold text-neutral-300 text-lg">No profiles found</p>
                                <p className="text-sm text-neutral-500 mt-1">
                                    {search || subSectorFilter !== 'all' || sourceFilter !== 'all' ? 'Try adjusting your filters' : 'Run the scraper: python engine/scheduler.py --once --sector telecom'}
                                </p>
                            </td></tr>
                        ) : profiles.map(p => {
                            ...
                            return (
                                <tr key={p.id} onClick={() => setSelectedProfile(p)}
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="font-bold text-white text-sm group-hover:text-blue-300 transition-colors truncate max-w-[280px]">
                    ...
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b border-white/5 flex items-start justify-between">
                            <div>
                                ...
                                {selectedProfile.current_title && <p className="text-sm text-blue-400 font-bold mt-1">{selectedProfile.current_title}</p>}
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            {selectedProfile.sub_sector && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-neutral-300 font-medium">Sub-Sector: {SUB_SECTOR_LABELS[selectedProfile.sub_sector]?.label || selectedProfile.sub_sector}</span>
                                </div>
                            )}
