...
import {
    ...
    ArrowUpRight, ArrowDownRight, Wrench, Filter,
...
const SOURCE_CONFIG: Record<string, { label: string; color: string }> = {
    ...
    rigzone: { label: 'Rigzone', color: 'emerald' },
    oiljobfinder: { label: 'OilJobFinder', color: 'amber' },
    energyjobline: { label: 'EnergyJobline', color: 'purple' },
    careerbuilder: { label: 'CareerBuilder', color: 'pink' },
    directory_seed: { label: 'Directory', color: 'neutral' },
    jsearch: { label: 'JSearch', color: 'cyan' },
};
...
export default function OilGasPage() {
    ...
    const downloadCSV = () => {
        ...
        const headers = ['Company', 'Title', 'Location', 'Sub-Sector', 'Email', 'Phone', 'Sources', 'Added'];
        const csvContent = [
            headers.join(','),
            ...profiles.map(p => [
        ...
        link.setAttribute('download', `oil_gas_profiles_${subSectorFilter}_${sourceFilter}.csv`);
    ...
    const getSubSectorStyle = (sub: string) => {
        const styles: Record<string, { bg: string; border: string; text: string }> = {
            upstream: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
            midstream: { bg: 'bg-blue-500/10', border: 'border-blue-500/20', text: 'text-blue-400' },
            downstream: { bg: 'bg-orange-500/10', border: 'border-orange-500/20', text: 'text-orange-400' },
            services: { bg: 'bg-purple-500/10', border: 'border-purple-500/20', text: 'text-purple-400' },
        };
    ...
    const getSourceStyle = (source: string) => {
        const styles: Record<string, { bg: string; border: string; text: string }> = {
            ...
            rigzone: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
            oiljobfinder: { bg: 'bg-amber-500/10', border: 'border-amber-500/20', text: 'text-amber-400' },
            energyjobline: { bg: 'bg-purple-500/10', border: 'border-purple-500/20', text: 'text-purple-400' },
            careerbuilder: { bg: 'bg-pink-500/10', border: 'border-pink-500/20', text: 'text-pink-400' },
            directory_seed: { bg: 'bg-neutral-500/10', border: 'border-neutral-500/20', text: 'text-neutral-400' },
            jsearch: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/20', text: 'text-cyan-400' },
        };
                        ...
                        {loading ? Array(8).fill(0).map((_, i) => (
                            ...
                            <tr><td colSpan={5} className="px-6 py-20 text-center">
                                ...
                                <p className="text-sm text-neutral-500 mt-1">
                                    {search || subSectorFilter !== 'all' || sourceFilter !== 'all' ? 'Try adjusting your filters' : 'Run the scraper: python engine/scheduler.py --once --sector oil_gas'}
                                </p>
                            </td></tr>
                        ) : profiles.map(p => {
