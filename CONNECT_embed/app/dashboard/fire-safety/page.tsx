...
import {
    Flame, Building2, MapPin, Mail, Phone, Search,
    ChevronLeft, ChevronRight, X, Briefcase, Shield, Bell,
    Droplets, Siren, Award, RefreshCw, Clock, Download, Activity
} from 'lucide-react';
...
const SUB_SECTOR_LABELS: Record<string, { label: string; icon: any; color: string }> = {
    fire_alarm: { label: 'Fire Alarms', icon: Bell, color: 'red' },
    sprinklers: { label: 'Sprinkler Systems', icon: Droplets, color: 'blue' },
    suppression: { label: 'Fire Suppression', icon: Siren, color: 'orange' },
    firestopping: { label: 'Firestopping', icon: Shield, color: 'amber' },
    life_safety: { label: 'Life Safety & Code', icon: Award, color: 'emerald' },
};

const SOURCE_CONFIG: Record<string, { label: string; color: string }> = {
    indeed: { label: 'Indeed', color: 'blue' },
    directory_seed: { label: 'Directory', color: 'neutral' },
    afsa_directory: { label: 'AFSA', color: 'emerald' },
    nfsa_directory: { label: 'NFSA', color: 'cyan' },
};
...
export default function FireSafetyPage() {
    ...
    const [stats, setStats] = useState({
        ...
        fire_alarm: 0, sprinklers: 0, suppression: 0, firestopping: 0, life_safety: 0,
    });
    ...
    const fetchStats = async () => {
        const [
            ...
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }),
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }).not('email', 'is', null).neq('email', ''),
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }).not('phone', 'is', null).neq('phone', ''),
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'fire_alarm'),
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'sprinklers'),
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'suppression'),
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'firestopping'),
            supabase.from('fire_safety_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'life_safety'),
        ]);

        setStats({
            total: total || 0, withEmail: withEmail || 0, withPhone: withPhone || 0,
            fire_alarm: fire_alarm || 0, sprinklers: sprinklers || 0,
            suppression: suppression || 0, firestopping: firestopping || 0,
            life_safety: life_safety || 0,
        });

        const { data: allSources } = await supabase.from('fire_safety_profiles').select('sources');
        if (allSources) {
    ...
    const fetchProfiles = useCallback(async () => {
        setLoading(true);
        let query = supabase.from('fire_safety_profiles').select('*', { count: 'exact' });
    ...
    const downloadCSV = () => {
        ...
        link.setAttribute('download', `fire_safety_profiles_${subSectorFilter}_${sourceFilter}.csv`);
    ...
    const getSubSectorStyle = (sub: string) => {
        const styles: Record<string, { bg: string; border: string; text: string }> = {
            fire_alarm: { bg: 'bg-red-500/10', border: 'border-red-500/20', text: 'text-red-400' },
            sprinklers: { bg: 'bg-blue-500/10', border: 'border-blue-500/20', text: 'text-blue-400' },
            suppression: { bg: 'bg-orange-500/10', border: 'border-orange-500/20', text: 'text-orange-400' },
            firestopping: { bg: 'bg-amber-500/10', border: 'border-amber-500/20', text: 'text-amber-400' },
            life_safety: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
        };
    ...
    const getSourceStyle = (source: string) => {
        const styles: Record<string, { bg: string; border: string; text: string }> = {
            ...
            afsa_directory: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
            nfsa_directory: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/20', text: 'text-cyan-400' },
        };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                        ...
                        Fire & Safety Recruiting
                    </h1>
                    <p className="text-neutral-400 font-medium mt-2">
                        {stats.total.toLocaleString()} profiles — alarms, sprinklers, suppression & life safety
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={downloadCSV}
                        className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-lg bg-red-500 text-white hover:bg-red-400 transition-colors shadow-lg shadow-red-500/20"
                    >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'TOTAL PROFILES', value: stats.total, icon: Building2, color: 'red' },
                    ...
                    { label: 'FIRE ALARMS', value: stats.fire_alarm, icon: Bell, color: 'amber' },
                ].map(s => (
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-2 overflow-x-auto pb-1">
                    <span className="text-[10px] font-black text-neutral-500 uppercase tracking-wider shrink-0 mr-1">Type:</span>
                    <button
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder="Search by company, title, or location..."
                        ...
                        className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-medium placeholder:text-neutral-600 focus:outline-none focus:border-red-500/50 focus:ring-1 focus:ring-red-500/20 transition" />
                </div>
                {(subSectorFilter !== 'all' || sourceFilter !== 'all' || search) && (
                    <button
                        onClick={() => { setSubSectorFilter('all'); setSourceFilter('all'); setSearch(''); setPage(0); }}
                        className="text-xs font-bold text-red-400 hover:text-red-300 px-4 py-2 bg-red-500/10 rounded-xl border border-red-500/20 transition-colors whitespace-nowrap"
                        ...
                        {loading ? Array(8).fill(0).map((_, i) => (
                            ...
                            <tr><td colSpan={5} className="px-6 py-20 text-center">
                                ...
                                <p className="text-sm text-neutral-500 mt-1">
                                    {search || subSectorFilter !== 'all' || sourceFilter !== 'all' ? 'Try adjusting your filters' : 'Run: python engine/scheduler.py --once --sector fire_safety'}
                                </p>
                            </td></tr>
                        ) : profiles.map(p => {
                            ...
                            return (
                                <tr key={p.id} onClick={() => setSelectedProfile(p)}
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="font-bold text-white text-sm group-hover:text-red-300 transition-colors truncate max-w-[280px]">
                    ...
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b border-white/5 flex items-start justify-between">
                            <div>
                                ...
                                {selectedProfile.current_title && <p className="text-sm text-red-400 font-bold mt-1">{selectedProfile.current_title}</p>}
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            {selectedProfile.sub_sector && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-neutral-300 font-medium">Type: {SUB_SECTOR_LABELS[selectedProfile.sub_sector]?.label || selectedProfile.sub_sector}</span>
                                </div>
                            )}
