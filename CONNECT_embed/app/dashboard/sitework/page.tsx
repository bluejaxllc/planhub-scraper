...
import {
    Tractor, Building2, MapPin, Mail, Phone, Search,
    ChevronLeft, ChevronRight, X, Briefcase, Shield,
    RefreshCw, Clock, Download, Activity,
    Zap, Construction, Truck, HardHat, Droplet, Trees
} from 'lucide-react';
...
const SUB_SECTOR_LABELS: Record<string, { label: string; icon: any; color: string }> = {
    excavation: { label: 'Excavation & Earthwork', icon: Tractor, color: 'orange' },
    paving: { label: 'Paving & Asphalt', icon: Truck, color: 'neutral' },
    demolition: { label: 'Demolition', icon: Construction, color: 'red' },
    utilities: { label: 'Underground Utilities', icon: Zap, color: 'yellow' },
    retaining: { label: 'Retaining Walls', icon: Shield, color: 'amber' },
    landscaping: { label: 'Landscaping & Irrigation', icon: Trees, color: 'emerald' },
};
...
export default function SiteworkPage() {
    ...
    const [stats, setStats] = useState({
        ...
        excavation: 0, paving: 0, demolition: 0, utilities: 0, retaining: 0, landscaping: 0
    });
    ...
    const fetchStats = async () => {
        const [
            ...
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).not('email', 'is', null).neq('email', ''),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).not('phone', 'is', null).neq('phone', ''),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'excavation'),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'paving'),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'demolition'),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'utilities'),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'retaining'),
            supabase.from('sitework_profiles').select('*', { count: 'exact', head: true }).eq('sub_sector', 'landscaping'),
        ]);

        setStats({
            total: total || 0, withEmail: withEmail || 0, withPhone: withPhone || 0,
            excavation: excavation || 0, paving: paving || 0,
            demolition: demolition || 0, utilities: utilities || 0,
            retaining: retaining || 0, landscaping: landscaping || 0,
        });

        const { data: allSources } = await supabase.from('sitework_profiles').select('sources');
        if (allSources) {
    ...
    const fetchProfiles = useCallback(async () => {
        setLoading(true);
        let query = supabase.from('sitework_profiles').select('*', { count: 'exact' });
    ...
    const downloadCSV = () => {
        ...
        link.setAttribute('download', `sitework_profiles_${subSectorFilter}_${sourceFilter}.csv`);
    ...
    const getSubSectorStyle = (sub: string) => {
        const styles: Record<string, { bg: string; border: string; text: string }> = {
            excavation: { bg: 'bg-orange-500/10', border: 'border-orange-500/20', text: 'text-orange-400' },
            paving: { bg: 'bg-neutral-500/10', border: 'border-neutral-500/20', text: 'text-neutral-400' },
            demolition: { bg: 'bg-red-500/10', border: 'border-red-500/20', text: 'text-red-400' },
            utilities: { bg: 'bg-yellow-500/10', border: 'border-yellow-500/20', text: 'text-yellow-400' },
            retaining: { bg: 'bg-amber-500/10', border: 'border-amber-500/20', text: 'text-amber-400' },
            landscaping: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
        };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                        <Tractor className="w-8 h-8 text-orange-400" />
                        Sitework & Civil
                    </h1>
                    <p className="text-neutral-400 font-medium mt-2">
                        {stats.total.toLocaleString()} profiles — excavation, paving, demolition & utilities
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={downloadCSV}
                        className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-lg bg-orange-500 text-white hover:bg-orange-400 transition-colors shadow-lg shadow-orange-500/20"
                    >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'TOTAL PROFILES', value: stats.total, icon: Building2, color: 'orange' },
                    ...
                    { label: 'EXCAVATION', value: stats.excavation, icon: Tractor, color: 'amber' },
                ].map(s => (
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder="Search by company, title, or location..."
                        ...
                        className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-medium placeholder:text-neutral-600 focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20 transition" />
                </div>
                {(subSectorFilter !== 'all' || sourceFilter !== 'all' || search) && (
                    <button
                        onClick={() => { setSubSectorFilter('all'); setSourceFilter('all'); setSearch(''); setPage(0); }}
                        className="text-xs font-bold text-orange-400 hover:text-orange-300 px-4 py-2 bg-orange-500/10 rounded-xl border border-orange-500/20 transition-colors whitespace-nowrap"
            ...
            <div className="bg-[#070707]/90 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden">
                <table className="w-full">
                    <thead><tr className="border-b border-white/5">
                        ...
                        <th className="px-4 py-4 text-left text-[10px] font-black uppercase tracking-[0.15em] text-neutral-500 hidden lg:table-cell">Sector</th>
                    ...
                    <tbody className="divide-y divide-white/[0.03]">
                        {loading ? Array(8).fill(0).map((_, i) => (
                            ...
                            <tr><td colSpan={5} className="px-6 py-20 text-center">
                                <Tractor className="w-12 h-12 text-neutral-700 mx-auto mb-4" />
                                <p className="font-bold text-neutral-300 text-lg">No profiles yet</p>
                                <p className="text-sm text-neutral-500 mt-1">
                                    {search || subSectorFilter !== 'all' || sourceFilter !== 'all' ? 'Try adjusting your filters' : 'Run: python engine/scheduler.py --once --sector sitework'}
                                </p>
                            </td></tr>
                        ) : profiles.map(p => {
                            ...
                            return (
                                <tr key={p.id} onClick={() => setSelectedProfile(p)}
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="font-bold text-white text-sm group-hover:text-orange-300 transition-colors truncate max-w-[280px]">
                    ...
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b border-white/5 flex items-start justify-between">
                            <div>
                                ...
                                {selectedProfile.current_title && <p className="text-sm text-orange-400 font-bold mt-1">{selectedProfile.current_title}</p>}
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            {selectedProfile.sub_sector && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-neutral-300 font-medium">Sector: {SUB_SECTOR_LABELS[selectedProfile.sub_sector]?.label || selectedProfile.sub_sector}</span>
                                </div>
                            )}
